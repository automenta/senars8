...
describe('Task - Edge Cases', () => {
    test('should handle invalid truth value objects with NaN values', () => {
        const term = new Term('cat');
        const truthValue = {frequency: NaN, confidence: 0.5};
    ...
    test('should handle invalid truth value objects with Infinity values', () => {
        const term = new Term('cat');
        const truthValue = {frequency: Infinity, confidence: 0.5};
        const task = new Task(term, '.', truthValue);
    ...
    test('should handle invalid truth value objects with negative values', () => {
        const term = new Term('cat');
        const truthValue = {frequency: -0.5, confidence: -0.2};
        const task = new Task(term, '.', truthValue);
        // Should clamp to valid range
    ...
    test('should handle invalid truth value objects with values > 1', () => {
        const term = new Term('cat');
        const truthValue = {frequency: 1.5, confidence: 1.2};
        const task = new Task(term, '.', truthValue);
        // Should clamp to valid range
        expect(task.state.truthValue.frequency).toBe(1.0);
    ...
    test('should handle malformed term strings', () => {
        expect(() => new Task('(invalid', '.')).toThrow('Failed to parse term');
        expect(() => new Task('also_invalid)', '.')).toThrow('Failed to parse term');
        expect(() => new Task('', '.')).toThrow('Invalid Task arguments');
    });

    test('should handle invalid punctuation', () => {
        const term = new Term('cat');
        expect(() => new Task(term, 'x')).toThrow('Invalid Task arguments');
        expect(() => new Task(term, '')).toThrow('Invalid Task arguments');
        expect(() => new Task(term, null)).toThrow('Invalid Task arguments');
    });

    test('should handle null and undefined terms', () => {
        expect(() => new Task(null, '.')).toThrow('Invalid Task arguments');
        expect(() => new Task(undefined, '.')).toThrow('Invalid Task arguments');
        expect(() => new Task('', '.')).toThrow('Invalid Task arguments');
    });
