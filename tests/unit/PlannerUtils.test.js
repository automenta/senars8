...
describe('PlannerUtils', () => {
    ...
    describe('findDecompositionMethods', () => {
        it('should return methods from the implication index', () => {
            ...
            memory.indexer.implicationIndex.set(goalTerm.key, methods);
    ...
    describe('isAchieved', () => {
        it('should return true if a belief meets the confidence threshold', () => {
            ...
            memory.indexer.beliefIndex.set(term.key, [{state: {truthValue: {confidence: 0.95}}}]);
        ...
        it('should return false if a belief is below the confidence threshold', () => {
            ...
            memory.indexer.beliefIndex.set(term.key, [{state: {truthValue: {confidence: 0.5}}}]);
            const result = PlannerUtils.isAchieved(term, memory, config);
    ...
    describe('arePreconditionsMet', () => {
        it('should return true if all preconditions are met', () => {
            ...
            memory.indexer.beliefIndex.set(pre1.key, [{state: {truthValue: {confidence: 0.85}}}]);
            memory.indexer.beliefIndex.set(pre2.key, [{state: {truthValue: {confidence: 0.9}}}]);
        ...
        it('should return false if any precondition is not met', () => {
            ...
            memory.indexer.beliefIndex.set(pre1.key, [{state: {truthValue: {confidence: 0.9}}}]);
            memory.indexer.beliefIndex.set(pre2.key, [{state: {truthValue: {confidence: 0.7}}}]);
            const result = PlannerUtils.arePreconditionsMet([pre1, pre2], memory, config);
            expect(result).toBe(false);
        });
