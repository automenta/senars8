...
function createConflict(type, task1, task2, details) {
    return {
        type,
...
function analyzeNegationConflict(task1, task2, parsed1, parsed2, type, conflictType) {
    ...
    if (task1.termKey === negatedTermKey) {
        return createConflict(conflictType, task1, task2, `Conflict between "${task1.termKey}" and its negation.`);
    }
    return null;
}
...
function analyzeSetLikeConflict(task1, task2, parsed1, parsed2, type, conflictType, details) {
    ...
    if (terms1.size !== terms2.size || ![...terms1].every(t => terms2.has(t))) {
        return createConflict(conflictType, task1, task2, details);
    }
    return null;
}

export {
    analyzeNegationConflict,
