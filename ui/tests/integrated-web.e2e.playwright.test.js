...
test.describe('Integrated Web UI End-to-End Tests', () => {
    ...
    test.beforeAll(async () => {
        ...
        integratedProcess.stdout.on('data', (data) => {
            ...
            console.log('Integrated runner:', output.trim());

            // Look for the server URLs in the output
            const httpMatch = output.match(/Local:\s+(http:\/\/localhost:(\d+))/);
            if (httpMatch) {
                serverUrl = httpMatch[1];
                console.log('Web UI available at:', serverUrl);
            } else {
                // Try alternative pattern for Vite output
                const altMatch = output.match(/http:\/\/localhost:(\d+)/);
                if (altMatch) {
            ...
            const wsMatch = output.match(/WebSocket server running on port:\s+(\d+)/);
            if (wsMatch) {
