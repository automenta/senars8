...
// Description: Showcases enhanced perception, processing various event types into tasks.
async function enhancedPerceptionDemo() {
    const taskDefs = [{
        termKey: '(perception_system_active)',
        punctuation: '.',
        truthValue: {
            frequency: 1.0,
            confidence: 0.95
        ...
        termKey: '(enhanced_perception_available)',
        punctuation: '.',
        truthValue: {
            frequency: 1.0,
            confidence: 0.95
    ...
    const preCycleCallback = async (system) => {
        ...
        system.perception.registerSensoryModality('visual', async (input) => {
            ...
            const visualTask = new Task(
                ...
                '.', {
                    frequency: input.confidence || 0.9,
                    confidence: 0.8
                }
            );
        ...
        system.perception.registerSensoryModality('auditory', async (input) => {
            const tasks = [];
