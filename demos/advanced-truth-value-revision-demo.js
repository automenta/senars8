// Description: Demonstrates advanced truth value revision capabilities.
const {
    createTask,
    runDemo
} = require('./demo-utils');
...
async function advancedTruthValueRevisionDemo() {
    const taskDefs = [
        ...
        // Goals for revision
        {
            termKey: 'resolve_bird_flying_contradictions',
            punctuation: '!',
            truthValue: {
                frequency: 1.0,
                confidence: 0.9
    ...
    const postCycleCallback = () => {
        ...
        console.log("\n1. Bayesian revision:");
        try {
            const testTask = createTask('(test_belief)', '.', {
                frequency: 0.7,
                confidence: 0.6
            });
            if (testTask) {
                ...
                truthValueManager.bayesianRevision(testTask, {
                    frequency: 0.9,
                    confidence: 0.8
                }, 0.7);
        ...
        try {
            const testTask = createTask('(consensus_test)', '.', {
                frequency: 0.5,
                confidence: 0.5
            });
            if (testTask) {
        ...
        try {
            ...
            const testTask = createTask('(temporal_test)', '.', {
                frequency: 0.9,
                confidence: 0.9
            }, oldStamp);
            if (testTask) {
        ...
        try {
            const conflictingTask1 = createTask('(conflict_test)', '.', {
                frequency: 0.9,
                confidence: 0.8
            });
            const conflictingTask2 = createTask('(conflict_test)', '.', {
                frequency: 0.2,
                confidence: 0.7
            });
        ...
        try {
            const testTask = createTask('(reinforcement_test)', '.', {
                frequency: 0.6,
                confidence: 0.5
            });
            if (testTask) {
        ...
        try {
            const testTask = createTask('(sophisticated_test)', '.', {
                frequency: 0.7,
                confidence: 0.6
            });
            if (testTask) {
