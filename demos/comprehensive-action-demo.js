...
async function comprehensiveActionDemo() {
    ...
    const simpleActionHandlers = [
        { name: 'move_*', handler: async (action) => info(`Action: Moving robot to ${action.name.replace('move_robot_', '')}`) },
        { name: 'activate_*', handler: async (action) => info(`Action: Activating ${action.name.replace('activate_', '')}`) },
    ];
    ...
    await runDemo('Complex Action Execution', complexActionTasks, { cycleCount: 4, actionHandlers: complexActionHandlers });
