...
async function enhancedNarseseDemo() {
    ...
    const taskDefs = [
        ...
        { termKey: '((task_a --> start) until (task_b --> finish))', punctuation: '.', truthValue: { frequency: 0.85, confidence: 0.8 } },
        { termKey: '((event_a --> occur) since (event_b --> happen))', punctuation: '.', truthValue: { frequency: 0.75, confidence: 0.7 } },
        
        // Complex conjunctions
        { termKey: '(&/, (initialize --> system), (load --> data), (process --> information))', punctuation: '.', truthValue: { frequency: 0.9, confidence: 0.85 } },
        { termKey: '(&|, (monitor --> cpu), (monitor --> memory), (monitor --> network))', punctuation: '.', truthValue: { frequency: 0.85, confidence: 0.8 } },
        
        // Nested temporal expressions
        { termKey: '(always, (eventually, (goal --> achieve)))', punctuation: '.', truthValue: { frequency: 0.9, confidence: 0.75 } },
        { termKey: '(next, (previous, (state --> previous)))', punctuation: '.', truthValue: { frequency: 0.8, confidence: 0.6 } },
        
        // Questions about temporal relationships
        { termKey: '(next, (action --> take))', punctuation: '?', truthValue: { frequency: 1.0, confidence: 0.8 } },
        { termKey: '((process --> start) until (process --> finish))', punctuation: '?', truthValue: { frequency: 1.0, confidence: 0.8 } },
        
        // Goals with temporal constraints
        { termKey: '(eventually, (project --> complete))', punctuation: '!', truthValue: { frequency: 1.0, confidence: 0.9 } },
        { termKey: '(&/, (acquire --> resources), (next, (begin --> implementation)))', punctuation: '!', truthValue: { frequency: 1.0, confidence: 0.9 } }
    ];
